...
console.log("FFprobe exists:", fs.existsSync(FFPROBE_PATH));
console.log("==================================\n");
...
function buildSrtForRange(
    ...
    // Filter and sort all valid segments in range chronologically
    ...
    for (let i = 0; i < validSegments.length; i += 1) {
        ...
        // Deduplicate identical adjacent text
        if (
            i > 0 &&
            validSegments[i - 1].text.toLowerCase() ===
                segment.text.toLowerCase() &&
            Math.abs(segment.start - validSegments[i - 1].start) < 2.0
        ) {
            continue;
        }

        let relStart = Math.max(
            0,
            segment.start - startLimit
        );

        let relEnd = Math.min(
            clipDuration,
            segment.end - startLimit
        );

        // ENFORCE STRICT ZERO-OVERLAP GUARANTEE:
        // A subtitle cannot appear while the previous subtitle is still active
        if (relStart < lastRelEnd) {
            relStart = lastRelEnd;
        }

        // If end is at or before start, assign minimum duration
        if (relEnd <= relStart) {
            relEnd = Math.min(clipDuration, relStart + 0.8);
        }
