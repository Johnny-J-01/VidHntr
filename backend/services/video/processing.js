...
async function runYouTubePipeline(videoId, url) {
	try {
		...
		const result = await ytdlp.downloadYouTubeAudio(
			...
			async (progress) => {
				await store.patchVideo(videoId, {
					progress: Math.min(15, 2 + Math.round(progress * 0.13)),
				});
			},
		);
		...
		const currentVideo = await store.getVideo(videoId);

		const duration =
			Number(currentVideo?.duration) || Number(metadata?.duration) || 0;
		...
		await store.patchVideo(videoId, {
			...
			audioPath,
			duration,
			title: currentVideo?.title || metadata?.title || "YouTube video",
			status: "TRANSCRIBING",
			progress: 20,
		});

		await runAudioTranscriptionPipeline(videoId, audioPath, duration, null);
	} catch (error) {
...
export { DIRS, runProcessingPipeline, runYouTubePipeline };
